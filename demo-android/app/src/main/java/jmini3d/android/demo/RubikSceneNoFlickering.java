...
public class RubikSceneNoFlickering extends ParentScene {
	...
	Vector3 rotations[] = new Vector3[9 * 3];
	...
	public RubikSceneNoFlickering() {
		...
		for (int i = 0; i < rotations.length; i += 3) {
			rotations[i] = new Vector3(axis[1]);
			rotations[i + 1] = new Vector3(axis[0]);
			rotations[i + 2] = new Vector3(axis[2]);
		}
	...
	public void update() {
		...
		if (nextRotationAxis == -1) {
			...
			for (Object3d piece : rotationGroup.getChildren()) {
				...
				rotate(nextRotationAxis, (float) Math.toRadians(angle), pos);
